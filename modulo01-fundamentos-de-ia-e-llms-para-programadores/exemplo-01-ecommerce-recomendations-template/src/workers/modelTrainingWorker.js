...
// Função de normalização para escalar os valores de idade e preço entre 0 e 1
...
function makeContext(products, users) {
  ...
  // ajuda a personalizar as recomendações com base na faixa etária dos usuários que compraram um produto específico.
  ...
  return {
    ...
    minPrice,
    maxPrice,
    categoriesQty: categories.length,
...
function encodeProduct(product, context) {
  // normalizando dados para ficar entre 0 e 1, e aplicando pesos
...
function encodeUser(user, context) {
  ...
  return tf
    .concat1d([
      ...
      tf.zeros([context.categoriesQty]), // categoria ignorada,
      tf.zeros([context.colorsQty]), // color ignorada,
    ])
    .reshape([1, context.dimentions]);
}
...
function createTrainData(context) {
    ...
    .forEach((user) => {
      ...
      context.products.forEach((product) => {
        ...
        const label = user.purchases.some(
          ...
          ? 1
          : 0;
        // combinando users + produtos para criar os dados de treino, onde cada linha representa um par user-product e o label indica se o usuário comprou ou não o produto.
  ...
  return {
    ...
    inputDimention: context.dimentions * 2, // considerando user + product
  };
}
